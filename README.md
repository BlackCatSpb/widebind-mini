...
## 3. Параметры (D=896, L=12, G=8, bind_twist_mode=off)
...
При `--bind-twist-mode shift`: BottleneckBind → 1,723,392 (S=4 multi-ocular), total ~12.23M.
...
Активный режим: `shift` с `--private-mem`.
...
| **Gate EMA fix** | `.data.mul_()` вместо `.mul_()` — in-place мутация ломала autograd (версия графика → NaN градиенты) |
| **Aux weights reduced** | `div_weight=0.0001` (было 0.01), `balance=0.001`, `reinforce=0.001`. `div_loss` переведён на `.var()`. |
| **Loss monitoring** | `_cached_losses` dict в compute_loss — каждый aux loss логируется отдельно |
| **debug_mind()** | gate_selectivity, behavior_div, concept_sim, spec_index, cons_index для визуализации специализации |
...
1. Полноценный запуск на T4 (Colab) с `--private-mem`
2. Мониторинг contra_expert, trust_matrix, spec_index через debug_mind()
